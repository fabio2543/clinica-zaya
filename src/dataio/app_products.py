...
def _validate_rows(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in out.iterrows():
        ...
        for col in required:
            if (
                pd.isna(row.get(col, None))
                or str(row.get(col)).strip() == ""
                or (col in ["unit_price", "quantity"] and float(row.get(col, 0)) < 0)
            ):
    ...
    try:
        ...
        for c in SCHEMA_COLUMNS:
            if c not in df.columns:
                df[c] = (
                    ""
                    if c not in ["unit_price", "quantity", "is_deleted"]
                    else (
                        0.0 if c == "unit_price" else (0 if c == "quantity" else False)
                    )
                )
...
menu = st.sidebar.radio(
    "Menu", ["Consulta", "Inserção", "Atualização", "Exclusão", "Carga (CSV/XLSX)"]
)
...
if menu == "Consulta":
    ...
    if q_name.strip():
        ...
        q = q[
            q["product_name"].str.lower().str.contains(s)
            | q["sku"].str.lower().str.contains(s)
        ]
    if status_filter != "Todos":
        q = q[q["status"].str.lower() == status_filter]
    if hide_deleted:
        q = q[~q["is_deleted"].fillna(False)]
    ...
    with exp2:
        buf = io.BytesIO()
        with pd.ExcelWriter(
            buf, engine="xlsxwriter", datetime_format="yyyy-mm-dd"
        ) as writer:
            q.to_excel(writer, index=False, sheet_name="Produtos")
        st.download_button(
            "Exportar Excel", data=buf.getvalue(), file_name="produtos_export.xlsx"
        )
    ...
    if base.empty:
        ...
        choices = base.apply(
            lambda r: f"{r['product_name']} | {r['sku']} | {r['purchase_date']} | id={r['id']}",
            axis=1,
        ).tolist()
        ...
        with st.form("form_update"):
            ...
            unit_price = st.number_input(
                ...
                min_value=0.0,
                step=0.01,
                value=float(rec["unit_price"]),
                format="%.2f",
            )
            purchase_date = st.date_input(
                "Data da compra *", value=pd.to_datetime(rec["purchase_date"]).date()
            )
            quantity = st.number_input(
                "Quantidade *", min_value=0, step=1, value=int(rec["quantity"])
            )
            ...
            status = st.selectbox(
                "Status",
                ["active", "inactive"],
                index=0 if rec["status"] == "active" else 1,
            )
    ...
    st.caption(
        "Aceita CSV ou Excel com as colunas: product_name, unit_price, purchase_date, quantity, sku, category, notes, status"
    )
    ...
    if up is not None:
        try:
            ...
            if not errs.empty:
                st.error(
                    f"Foram encontrados {errs.shape[0]} erros. Corrija e reenvie o arquivo."
                )
            else:
                ...
                to_add = raw.copy()
                to_add.insert(
                    0, "id", [str(uuid.uuid4()) for _ in range(to_add.shape[0])]
                )
                ...
                st.success(
                    f"Carga realizada com sucesso. Linhas inseridas: {to_add.shape[0]}"
                )
