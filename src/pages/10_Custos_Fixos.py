...
category = cols[1].selectbox(
    "Categoria", cat_options, index=0, key="fx_categoria_filter"
)
cols[2].checkbox("Mostrar excluídos", value=False, disabled=True)
buscar = cols[3].button("🔍 Buscar", use_container_width=True, key="fx_buscar_btn")
...
if df.empty and buscar:
    ...
    # Seleção robusta (sem NaN/ausência de coluna)

    id_col = "fixed_cost_id"
    ...
    if id_col in edited.columns:
        ...
        st.warning("Coluna 'fixed_cost_id' não está presente na grade.")
        selected_ids = []
    ...
    with c2:
        st.button(
            ...
            disabled=True,
            use_container_width=True,
            key="fx_mass_edit_btn",
        )

    with c3:
        if st.button("⬇️ Exportar CSV", use_container_width=True, key="fx_export_btn"):
...
with st.form("form_novo_custo_fixo"):
    ...
    f_category_sel = c3.selectbox(
        "Categoria",
        cat_opts,
        index=1 if len(cat_opts) > 1 else 0,
        key="fx_categoria_form",
    )
    ...
    payment_method_sel = c4.selectbox(
        "Forma de pagamento",
        pm_opts,
        index=1 if len(pm_opts) > 1 else 0,
        key="fx_payment_method_form",
    )
    vendor_sel = c5.selectbox(
        "Fornecedor",
        ven_opts,
        index=1 if len(ven_opts) > 1 else 0,
        key="fx_vendor_form",
    )
    cost_center_sel = c6.selectbox(
        "Centro de custo",
        cc_opts,
        index=1 if len(cc_opts) > 1 else 0,
        key="fx_cost_center_form",
    )
    ...
    due_day = st.number_input(
        "Dia de vencimento", min_value=1, max_value=31, step=1, key="fx_due_day"
    )
    notes = st.text_input("Observações", placeholder="", key="fx_notes")
...
up = st.file_uploader(
    "Envie CSV/XLSX no layout padrão", type=["csv", "xls", "xlsx"], key="fx_uploader"
)
if up:
    try:
        ...
        if missing:
            ...
            if st.button("Carregar arquivo", type="primary", key="fx_load_btn"):
...
dim_label = col1.selectbox(
    "Selecione a dimensão", list(dim_map.keys()), key="fx_dim_select"
)
dim_name, list_func = dim_map[dim_label]

items = list_func()
selected_item = col2.selectbox(
    "Item", items if items else ["(nenhum disponível)"], key="fx_dim_item"
)
...
if st.button(
    "🗑️ Executar ação", type="primary", use_container_width=True, key="fx_dim_action"
):
    try:
        from src.dataio.fixed_costs import pd as _pd2, DIM_DIR, utc_now

        path = DIM_DIR / f"dim_{dim_name}.parquet"
        df = _pd2.read_parquet(path)
        code_col = f"{dim_name}_code"
        is_active_col = "is_active"

        # Encontrar registro (case-insensitive)
        mask = (
            df[code_col].astype(str).str.lower() == str(selected_item).lower().strip()
        )
        if not mask.any():
